function [NewEvents rpktsend rpktrecv rEnergy] = action(event, log_file, simID,ppktsend, ppktrecv, pEnergy)
...
%             end
    ...
    case 'send_mac'
        ...
        if ~isempty(mac_queue(i).list) | mac_status(i)
            ...
            newevent.type = 'wait_for_channel';
            newevent.node = i;
% % %             newevent.pktsend =newevent.pktsend+1; %kabeer
% % %             newevent.Energy =newevent.Energy+1; %kabeer
...
%         end
    ...
    case 'backoff'
        ...
        i = event.node;
        j = event.pkt.rv;
        %SuccRatio=event.pkt.rv/event.pkt.tx;  %%% Imran
                SuccRatio=event.pkt.rv/backoff_attempt(i);  %%% Imran
        ...
        if node(i, 4) == 0 & carrier_sense(i) == 0
            ...
            if backoff_counter(i) > 1
                ...
                newevent.pktsend =newevent.pktsend+1; %kabeer
                newevent.Energy =newevent.Energy+0.33; %kabeer
                rpktsend=rpktsend+1;
                rEnergy=rEnergy+0.33;
...
%                 end
            ...
            if backoff_counter(i) > 1
                ...
                temp = min(backoff_attempt(i)+CW_min,CW_max);
                if simID==1
                 backoff_counter(i) = floor((2^temp-1)*rand); %%%default BEB counter
% disp('Hello')
%                 SuccRatio
%                 backoff_attempt(i)
                elseif simID==2
                    %%Fuzzy Logic Counter
                    if backoff_attempt(i)~=0
%                         [SuccRatio, backoff_attempt(i)]
                        ...
                        if backoff_attempt(i)<1
                            backoff_attempt(i)=1;
                        end
                        if backoff_attempt(i)>6
                            backoff_attempt(i)=6;
                        end
                        fismat=readfis('CR_nf');
                        backoff_counter(i)=evalfis([SuccRatio, backoff_attempt(i)],fismat);
                        %backoff_counter(i)=CWFuzzLogic(SuccRatio,backoff_attempt(i));
...
%                         [SuccRatio, backoff_attempt(i)]
                        ...
                        fismat=readfis('CR_nf');
                        backoff_counter(i)=evalfis([a(1), backoff_attempt(i)],fismat);
                        %backoff_counter(i)=CWFuzzLogic(a(1),backoff_attempt(i));
                    else
                        backoff_counter(i) = 0;
                    end
        ...
        switch event.pkt.type
            ...
            case 'data'
                ...
                if event.pkt.rv ~= 0    % send ACK if not broadcast
                    ...
                    newevent.pktrecv =newevent.pktrecv+1; %kabeer
                    newevent.Energy =newevent.Energy+0.1; %kabeer
                    rpktrecv=rpktrecv+1; 
                    rEnergy=rEnergy+0.1;
                    NewEvents = [NewEvents; newevent]; clear newevent;
                end
    ...
    case 'send_app'
        ...
        switch event.app.type
            case 'crosslayer_searching'
                ...
                %kabeer fprintf(fid, '%d %d %g %d %d %d \n', [event.app.id1; event.app.id2; t; 0; i; event.app.key]);
                %fprintf(fid, '%d %d %g %d %d %d %d %d %d\n', [event.app.id1; event.app.id2; t; 0; i; event.app.key; event.pktsend; event.pktrecv; event.Energy]);
                fprintf(fid, '%d %d %g %d %d %d %d %d %f \n', [event.app.id1; event.app.id2; t; 0; i; event.app.key; ppktsend; ppktrecv; pEnergy]);
    ...
    case 'recv_app'
        ...
        switch event.app.type
            case 'crosslayer_searching'
                ...
                %fprintf(fid, '%d %d %g %d %d %d %d %d %f \n', [event.app.id1; event.app.id2; t; length(event.net.route)-1; j; event.app.key; event.pktsend; event.pktrecv; event.Energy]);
                fprintf(fid, '%d %d %g %d %d %d %d %d %f \n', [event.app.id1; event.app.id2; t; length(event.net.route)-1; j; event.app.key; ppktsend; ppktrecv; pEnergy]);
            ...
            case 'dht_searching'
                ...
                elseif tempi == 1
                    ...
                    if fid == -1, error(['Cannot open file', log_file]); end
                    % Record traffic_id, topo_id, start_time, start_hop_count(=0), requesting node, requesting key, overlay route length
                    fprintf(fid, '%d %d %g %d %d %d %d %d %d %d\n', [event.app.id1; event.app.id2; t; event.app.hopcount; j; event.app.key; length(event.app.route)-1; event.pktsend; event.pktrecv; event.Energy]);
