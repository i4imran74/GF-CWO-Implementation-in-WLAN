...
interval = 5;  % second
itraffic = 5;
max_time = 5 + interval * (nrepeat + 1);  %changed time 500 to 5
for RunNumOfTimes=[1:2]  %Run  two times
for simID=[1:3]
%log_file = [log_file num2str(simID)];
log_file = ['log_mobility_crosslayer_' num2str(RunNumOfTimes) num2str(simID)];

for imobility = 4:4:nmobility
    ...
    for k=1:itraffic
        ...
        tempe.net = [];
        tempe.pkt = [];
            tempe.pktsend = 0; %kabeer
            tempe.pktrecv = 0; %kabeer
            tempe.Energy = 0; %kabeer        
        for h = 1:nrepeat
    ...
    % Run the simulation
    tstart = clock;
    run(Event_list', max_time, [log_file, num2str(maxspeed)],simID);
